...
| Touch panel | `nano/panel.yaml` + `nano/panel_api.py` | Firmware (ESPHome + LVGL) for the Waveshare ESP32-S3-Touch-LCD-7 and the Nano-side API it talks to on `:8765`. Wizard: *I speak…* → language → *Translate to…* → language, then the conversation screen (pair as sage/steel chips; tap a chip for whose turn it is). Phrases come from `GET /phrases`; tap one → the Nano says it in the patient's language (native recording → Piper → learner recording → English, spoken warning when it falls back, with a *Record it* offer). Mic → whisper-cli in the speaker's language → llama-server translation → spoken. Record mode files volunteer takes as batches + whisper drafts. Start screen shows Wi-Fi/Nano/speech/translator status and has hold-to-shut-down. The panel joins the Nano's **PhraseKit** hotspot when present (`tools/nano.sh hotspot on`), else the home LAN. Deploy the API with `tools/nano.sh panel` (boot cron starts it and llama-server). Flash once over USB: `pip install esphome`, copy `nano/secrets.yaml.example` → `nano/secrets.yaml`, `esphome run nano/panel.yaml`; later updates go over Wi-Fi. |
